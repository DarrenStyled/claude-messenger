...
// In-memory group chat history (last 100 broadcast messages)
const MSG_LIMIT = 100;
const msgHistory = [];
...
// Deduplicate a username against currently connected users
...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ username, avatar, personalMessage }) => {
    const name = uniqueName(username || 'User');
    users.set(socket.id, {
      username: name,
      status: 'online',
      avatar: avatar || null,
    ...
    // Send recent group chat history to the new joiner
    socket.emit('history', msgHistory.slice(-MSG_LIMIT));
  ...
  socket.on('message', ({ to, text, color, fontSize, bold, italic }) => {
    ...
    if (to) {
      // Private message — not stored in history
      io.to(to).emit('message', payload);
      socket.emit('message', { ...payload, self: true });
    } else {
      // Group/broadcast — save to history
      msgHistory.push(payload);
      if (msgHistory.length > MSG_LIMIT) msgHistory.shift();
